...
export default function AdminMusaitlikPage() {
  ...
  function toggleDay(i: number) {
    setFilterDays((prev) => { const n = new Set(prev); n.has(i) ? n.delete(i) : n.add(i); return n; });
  }
  ...
  return (
    /* Outer column: PageHeader + tek kutu */
    <div className="flex h-screen flex-col gap-3 overflow-hidden">

      {/* PageHeader — sabit yükseklik */}
      ...
      {/* ═══════════ TEK KUTU — flex-1 min-h-0 ═══════════ */}
      <div className="flex flex-1 min-h-0 overflow-hidden rounded-2xl border border-border/60 bg-white">

        {/* ── SOL %25: Uzman Seç ── */}
        <div className="flex w-[25%] min-w-[180px] max-w-[260px] flex-col border-r border-border/60">

          {/* Header */}
          <div className="flex shrink-0 items-center justify-between border-b border-border/60 bg-[#e6f0ee] px-4 py-2.5">
          ...
          {/* Renk göstergesi */}
          {checkedIds.size > 0 && (
            <div className="shrink-0 space-y-1.5 border-t border-border/50 px-3 py-2.5">
              {experts
                .filter((e) => checkedIds.has(e.id))
                .map((e) => (
                  <div key={e.id} className="flex items-center gap-2">
                    <span className="size-2 shrink-0 rounded-full" style={{ background: colorMap.get(e.id) }} />
        ...
        {/* ── SAĞ %75: Filter bar + Tablo ── */}
        <div className="flex flex-1 min-w-0 flex-col">

          {/* ── Filter bar — shrink-0 (sabit yükseklik) ── */}
          <div className="flex shrink-0 flex-wrap items-center gap-x-5 gap-y-2.5 border-b border-border/60 bg-white px-5 py-3">
              ...
              <div className="flex gap-1">
                {DAY_LABELS.map((label, i) => (
                  <button
                    ...
                    onClick={() => toggleDay(i)}
                    className={cn(
                      "min-w-[30px] cursor-pointer rounded-lg border px-2 py-1 text-[11px] font-semibold transition-colors duration-150",
                      filterDays.has(i)
              ...
              <div className="flex gap-1">
                {TIME_RANGE_OPTS.map(({ value, label, sub }) => (
                  <button
                    ...
                    className={cn(
                      "flex cursor-pointer items-baseline gap-1 rounded-lg border px-2.5 py-1 text-[11px] font-semibold transition-colors duration-150",
                      filterRange === value
                  ...
                  >
                    {label}
                    {sub && (
                      <span className={cn("text-[9px]", filterRange === value ? "text-white/70" : "text-muted-foreground/60")}>
                        {sub}
                      </span>
                    )}
                  </button>
            ...
            {activeFilterCount > 0 && (
              <button
                ...
                className="ml-auto flex cursor-pointer items-center gap-1.5 rounded-lg border border-amber-200 bg-amber-50 px-2.5 py-1 text-[11px] font-semibold text-amber-700 transition hover:bg-amber-100"
              >
                <X className="size-3" />
          ...
          {/* ── Tablo — flex-1 min-h-0, scroll burada ── */}
          <div className="flex flex-1 min-h-0 flex-col overflow-hidden">
            ...
            <div className="flex-1 min-h-0 overflow-auto">
              ...
              <div className="sticky top-0 z-10 flex border-b-2 border-border/60 bg-[#e6f0ee]">
                  ...
                  return (
                    <div
                      ...
                      className={cn(
                        ...
                        today ? "bg-primary/15" : "bg-[#e6f0ee]",
                      )}
                    >
                      <p className={cn("text-[10px] font-bold uppercase tracking-wider", today ? "text-primary" : "text-muted-foreground")}>
                        {DAY_LABELS[dowIdx]}
                      </p>
                      <p className={cn("mt-0.5 text-lg font-bold leading-none", today ? "text-primary" : "text-[#014a3e]")}>
                        {date.getDate()}
                      </p>
                    </div>
                  );
              ...
              {timeSlots.length === 0 ? (
                ...
                timeSlots.map((time) => (
                  <div
                    key={time}
                    className="flex border-b border-border/40 last:border-b-0 transition-colors duration-100 hover:bg-muted/20"
                  >
                    {/* Saat etiketi */}
                    <div className="flex w-16 shrink-0 items-center justify-center border-r border-border/50 bg-[#e6f0ee] px-1 py-2">
                      <p className="text-[11px] font-semibold tabular-nums text-[#014a3e]">{time}</p>
                    </div>
                            ...
                            {cell.map((avail) => {
                              ...
                              return (
                                <button
                                  ...
                                  className={cn(
                                    "flex w-full cursor-pointer items-center gap-1.5 rounded-md px-1.5 py-[4px] text-[11px] font-semibold",
                                ...
                                >
                                  ...
                                  {blocked && <Lock className="ml-auto size-3 shrink-0 text-red-400" />}
                                </button>
                              );
                            })}
                          </div>
            ...
            {/* Footer — sabit alt */}
            <div className="flex shrink-0 flex-wrap items-center gap-5 border-t border-border/40 bg-muted/20 px-4 py-2.5 text-[11px] text-muted-foreground">
              <div className="flex items-center gap-1.5">
                <span className="size-2.5 rounded-full bg-[#4d978b]" /> Müsait
              </div>
              <div className="flex items-center gap-1.5">
                <span className="size-2.5 rounded-full bg-red-400" /> Admin Kilitli
              </div>
              <p className="ml-auto">Chip&apos;e tıklayarak kilit durumunu değiştirin</p>
            </div>
          </div>
        </div>
      </div>
